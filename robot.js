...
export function makeCBRobot() {
  ...
  const legWer = new THREE.Mesh(legGeometry, cardboardMaterial);
  legWer.position.set(-legSep/2, legLen/2, 0);
  cardboardRobot.add(legWer);
  const legDer = new THREE.Mesh(legGeometry, cardboardMaterial);
  legDer.position.set(legSep/2, legLen/2, 0);
  cardboardRobot.add(legDer);

  //  胴体の作成
  const bodyGeometry
    = new THREE.BoxGeometry(bodyW, bodyH, bodyD);
  const body = new THREE.Mesh(bodyGeometry, cardboardMaterial);
  body.position.set(0, legLen+legLen/2, 0);
